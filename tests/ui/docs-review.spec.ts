  ...
  test.describe(`docs review: ${route.name}`, () => {
    test('renders without root overflow or serious accessibility violations', async ({ page }, testInfo) => {
      ...
      if (route.name === 'v3-fees') {
        await expect(page.getByText('has not activated yet', { exact: false })).toBeVisible();
...
test.describe('navigation regressions', () => {
  test('connects V3 trading, order management, and planned markets', async ({ page }) => {
    ...
    await page.locator('main a[href="/protocol/saucerswap-v3/markets#planned-contract-migration"]').click();
    await expect(page).toHaveURL(/\/protocol\/saucerswap-v3\/markets#planned-contract-migration$/);
    await expect(page.getByRole('heading', { level: 2, name: /Planned contract migration/ })).toBeVisible();
    await page.locator('main a[href="/protocol/saucerswap-v3/fees#approved-market-fee-schedule"]').click();
    await expect(page).toHaveURL(/\/protocol\/saucerswap-v3\/fees#approved-market-fee-schedule$/);
    await expect(page.getByRole('heading', { level: 2, name: /Approved market fee schedule/ })).toBeVisible();
  });
