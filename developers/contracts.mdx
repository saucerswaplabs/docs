...
V3 order matching runs off-chain with on-chain settlement through a reactor contract. The reactor address is not a static list entry: fetch it per environment from the public signing-domain endpoint, which returns the EIP-712 domain used for order signatures.

```bash
curl "https://orderbook-api.saucerswap.finance/signature/domain"
```
