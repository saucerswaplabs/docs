...
<Steps>
  ...
  <Step title="Build orders">
    ...
    <Warning>
      Quote the current book immediately before building a market order and do not submit an unfillable quote. Never use `"1"` as a market order's `outputAmount`: that removes practical price protection from the signed order. Keep all integer quantities as strings, and do not cast nonces, deadlines, or raw token amounts to JavaScript numbers before signing. See [Market quotes](/api-reference/orderbook/market-data#market-quotes).
    </Warning>
  </Step>
