...
<Info>
  Fee rates use pips: 1 pip = 1e-6 = 0.0001%, and 100 pips = 1 basis point. `takerFeePips` and `makerFeePips` express trading fee rates. `capFractionPips` instead expresses the maker rebate cap as a fraction of effective taker fees, using the same scale; it is not an additional trading fee. See [V3 fees and rebates](/protocol/saucerswap-v3/fees#maker-rebate-safety-cap).
</Info>
