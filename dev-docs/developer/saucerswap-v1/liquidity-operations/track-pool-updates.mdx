...
* [Polling Sync events for all pools](/developer/saucerswap-v1/liquidity-operations/track-pool-updates#polling-sync-events-for-all-pairs)

* [Polling Sync events for a pool](/developer/saucerswap-v1/liquidity-operations/track-pool-updates#polling-sync-events-for-a-pair)
...
<Tabs>
  ...
  <Tab title>
    ...
    const url = `${config.mirrorNodeBaseUrl}/api/v1/contracts/results/logs?${params}`;  
...
<Tabs>
  ...
  <Tab title>
    ...
    const url = `${config.mirrorNodeBaseUrl}/api/v1/contracts/${contractIdEvm}/results/logs?${params}`;  
    const response = await axios.get(url);
    const logs = response.data.logs;
...
[PreviousGet Pool Reserves](/developer/saucerswap-v1/liquidity-operations/get-pool-reserves)[NextAdding Liquidity](/developer/saucerswap-v1/liquidity-operations/adding-liquidity)
