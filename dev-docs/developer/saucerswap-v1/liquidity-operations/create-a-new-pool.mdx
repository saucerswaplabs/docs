...
* [Creating a new HBAR/token liquidity pool](/developer/saucerswap-v1/liquidity-operations/create-a-new-pool#creating-a-new-hbar-token-liquidity-pair)

* [Creating a new token/token liquidity pool](/developer/saucerswap-v1/liquidity-operations/create-a-new-pool#creating-a-new-token-token-liquidity-pair)

**Contract ID:** [SaucerSwapV1RouterV3](https://hashscan.io/mainnet/contract/0.0.3045981)

See [Pool creation fee](/developer/saucerswap-v1/liquidity-operations/pool-creation-fee) to get the current pool creation fee.

Ensure that the liquidity pool does not exist on-chain prior to creation of the new pool. See [Check for an existing pool](/developer/saucerswap-v1/liquidity-operations/check-if-a-pool-exists).

Increase the client's max token auto-association by one to receive the incoming LP token.

To add liquidity to an existing pool, see [Adding liquidity](/developer/saucerswap-v1/liquidity-operations/adding-liquidity).
...
EVM address of the token to pair with HBAR
...
[PreviousPool Creation Fee](/developer/saucerswap-v1/liquidity-operations/pool-creation-fee)[NextGet Pool Reserves](/developer/saucerswap-v1/liquidity-operations/get-pool-reserves)
