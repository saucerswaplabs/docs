...
<CardGroup cols="2">
  <Card title="Default">
    ...
    1. <Icon icon="circle-check" />   Appears by default in token modal scrollable list
    2. <Icon icon="circle-check" />   Searchable by token name, symbol or ID
    3. <Icon icon="circle-check" />   Appears with token icon and other misc info e.g. description
    4. <Icon icon="circle-check" />   Appears in APIs
    5. <Icon icon="circle-check" />   Eligible for yield farm & LARI
    6. 

    # Criteria

    1. 
    2. <Icon icon="circle-check" />   Token icon available either via token metadata or Davinci Pics
    3. <Icon icon="circle-check" />   Tokens with a V2 pool, or comprising the top 50 V1 pools containing at least 1 'common' token (e.g. HBAR-TokenB) - by aggregated TVL & volume - updated monthly
    4. <Icon icon="circle-check" />   'Common' tokens: HBAR, HBARX, SAUCE, XSAUCE, USDC, USDC[HTS], USDT[HTS], WBTC[HTS], WETH[HTS]
    5. aaa
    6. aaa
    7. aaa
    8. 
