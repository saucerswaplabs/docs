...
Token metadata can be added & updated using third party tools such as:
...
| "twitter"                 | string | Twitter/X handle (without the @)                                   |
...
SaucerSwap Labs supports metadata conforming to the [Hedera fungible token Metadata schema](https://hips.hedera.com/hip/hip-405#schema).
...
Token icon and information can be added to [Davinci Pics](https://davincigraph.io/#pics_home)

Note: adding or updating a token icon on Davinci Pics requires connecting the token's treasury account ID; or the account ID which executed the token create transaction, to Davinci to authenticate.
