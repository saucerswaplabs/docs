...
Traders pay a swap fee on V1 and V2: fixed 0.30% on V1, and 0.05%, 0.15%, 0.30%, or 1.00% on V2 depending on the pool. V1 sends 5/6 of the fee to liquidity providers and 1/6 to the protocol. Each V2 pool has a configured protocol share of 1/N, where 4 ≤ N ≤ 10, with the remainder going to liquidity providers. The protocol share funds [Unified BrewSaucer buybacks](/tokenomics/overview#unified-brewsaucer-buybacks-and-burn), allocated to xSAUCE, development, burn, and liquidity incentives. V3 order book fees work differently — see [V3 fees](/protocol/saucerswap-v3/fees).
