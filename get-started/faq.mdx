...
The **trade** page is the V3 central limit order book, where you place and manage limit or market orders. The **swap** page compares a V1/V2 AMM quote with an eligible V3 quote and selects a venue for your trade. See [Swap routing](/protocol/routing) for how quotes are compared.
...
Traders pay a swap fee on V1 and V2: fixed 0.30% on V1, and 0.05%, 0.15%, 0.30%, or 1.00% on V2 depending on the pool. V1 sends 5/6 of the fee to liquidity providers and 1/6 to BrewSaucer. V2 uses the same split by default: 5/6 (about 83.33%) to liquidity providers and 1/6 (about 16.67%) to BrewSaucer. The DAO can set a different split for each V2 pool through [governance](/governance/overview).

The protocol share funds [Unified BrewSaucer buybacks](/tokenomics/overview#unified-brewsaucer-buybacks-and-burn), allocated to xSAUCE, development, burn, and liquidity incentives. V3 order book fees work differently — see [V3 fees](/protocol/saucerswap-v3/fees).
...
The Infinity Pool receives 40% of SAUCE purchased through Unified BrewSaucer. Buybacks are funded by the protocol's share of V1/V2 swap fees, net V3 order book fees, and HBAR native-staking rewards earned by the WHBAR contract. The former devcut contribution to staking is now routed to burn. See [Single-sided staking](/protocol/single-sided-staking).
