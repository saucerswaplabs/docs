...
Traders pay the pool's fee tier on each swap. By default, 5/6 (about 83.33%) of that fee goes to LPs and 1/6 (about 16.67%) goes to BrewSaucer. The split is configurable for each pool under [DAO governance](/governance/overview), separately from the swap fee tier. A new-pool proposal can request a different split.

The protocol share funds [Unified BrewSaucer buybacks](/tokenomics/overview#unified-brewsaucer-buybacks-and-burn), with purchased SAUCE allocated to xSAUCE, development, burn, and liquidity incentives. Unlike V1, LPs earn fees directly in the pool's tokens — a USDC/HBAR position accrues claimable USDC and HBAR.

$$
\mathrm{APR}_{\mathrm{fees}} = \frac{V_{24} \times f \times s_{\mathrm{LP}}}{L_{\mathrm{bal}}} \times 365
$$

where $V_{24}$ is the 24-hour trading volume, $f$ is the pool's fee tier expressed as a decimal, $s_{\mathrm{LP}}$ is the LP share of fees (5/6 by default), and $L_{\mathrm{bal}}$ is liquidity aggregated over a [balanced range](/protocol/saucerswap-v2#balanced-approach). Use the pool's configured LP share and the same currency for volume and liquidity. The interface displays the 7-day average.
...
LARI receives a dedicated Masterchef allocation, separate from the DAO's allocation: 3,500 of 5,000 points during Phase 1, or 70% of base SAUCE emissions. Partner projects can also commit tokens to campaigns. The DAO sets each epoch's allocation and per-pool weights through governance; see [LARI weights](/protocol/saucerswap-v2/lari-weights) for published epoch distributions and [SAUCE tokenomics](/tokenomics/overview) for the emission rate.
