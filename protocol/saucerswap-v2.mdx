...
Highly stable pairs (Minimal price volatility)
...
Moderately stable pairs (Low to medium volatility)
...
Volatile pairs (Significant price swings)
...
Highly volatile pairs (Unpredictable price movements)
...
Fees APR=LBal​24h Volume ×(Fee×65​)​×365

Where:

* Fee can be 0.05%, 0.15%, 0.30%, or 1.00%.

* LBal​ represents the total liquidity (aggregate of positions) in a [balanced range](/protocol/saucerswap-v2#balanced-approach).
...
For a position of size L, defined within the tick interval \[*a, b*] and containing the active tick, we determine its liquidity at that tick, Lpool, pos​, using the formula:
...
Tpool, pos​=j=1∑n​Spool, pos, j​

Where:

* Tpool, pos​ represents the total liquidity hours for a position during an epoch.

* Spool, pos, j​ represents the liquidity hours for each interval j, from j=1 to n.
...
Rpool, pos​=Tpool​Rpool​×Tpool, pos​​

Where:

* Rpool, pos​ represents the reward allocation, or the quantity of tokens earned by the position during an epoch.

* Rpool​ represents the reward allocation for the pool, found [here](/protocol/saucerswap-v2/lari-weights).

* Tpool, pos​ represents the total liquidity hours for the position during an epoch.

* Tpool​ represents the total liquidity hours for the pool, calculated by summing the total liquidity hours of every position Tpool, pos, j​, from j=1 to n, where n represents the total number of active positions in the pool throughout the epoch.

Tpool​=j=1∑n​Tpool, pos, j​

As an aside, the estimated Reward APR displayed in the SaucerSwap interface is calculated as follows:

Reward APR=Lpool​Rpool​×26.07145​

Where Lpool​ represents the pool liquidity.
...
LUSDC/USDT, Bob​=1010,000​=1,000

LUSDC/USDT, Alice​=1,00010,000​=10

In this example, we can assume Δt1​=336 hours because neither position falls out-of-range. Since there is only one interval, SUSDC/USDT, pos, 1​=TUSDC/USDT, pos​. The total liquidity hours for each position are therefore calculated as follows:

TUSDC/USDT, Bob​=1,000×336=336,000

TUSDC/USDT, Alice​=10×336=3,360

We can now determine the reward allocation in SAUCE for each position:

RUSDC/USDT, Bob​=336,000+3,360100,000×336,000​=99,010

RUSDC/USDT, Alice​=336,000+3,360100,000×3,360​=990
...
[PreviousFarm Weights](/protocol/saucerswap-v1/farm-weights)[NextLARI Weights](/protocol/saucerswap-v2/lari-weights)
