...
Traders pay the pool's fee tier on each swap. The configured protocol fee-switch share is 1/N, where 4 ≤ N ≤ 10; the remainder goes to LPs. The protocol share funds [Unified BrewSaucer buybacks](/tokenomics/overview#unified-brewsaucer-buybacks-and-burn), with purchased SAUCE allocated to xSAUCE, development, burn, and liquidity incentives. Unlike V1, LPs earn fees directly in the pool's tokens — a USDC/HBAR position accrues claimable USDC and HBAR.

$$
\mathrm{APR}_{\mathrm{fees}} = \frac{V_{24} \times f \times (1 - 1/N)}{L_{\mathrm{bal}}} \times 365
$$

where $V_{24}$ is the 24-hour trading volume, $f$ is the pool's fee tier expressed as a decimal, and $L_{\mathrm{bal}}$ is liquidity aggregated over a [balanced range](/protocol/saucerswap-v2#balanced-approach). Use the same currency for volume and liquidity. The interface displays the 7-day average.
